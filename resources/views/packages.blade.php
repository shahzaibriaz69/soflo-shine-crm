...
@section('content')
    <div class="max-w-7xl mx-auto">
        ...
        @if(session('success'))
                <div class="bg-emerald-950/50 border border-emerald-800 text-emerald-400 px-4 py-3 rounded-xl mb-6 text-sm flex
                items-center gap-3">
                <span>✅</span>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($packages as $package)
            <div
                class="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col justify-between shadow-xl hover:border-slate-700 transition">
                <div>
                    <div class="flex justify-between items-start mb-4">
                        <span
                            class="px-3 py-1 text-xs font-semibold bg-blue-950/80 text-blue-400 border border-blue-800/40 rounded-full">
                            {{ $package->category ?? 'General' }}
                        </span>
                        <span class="text-xl font-bold text-emerald-400">
                            ${{ number_format($package->price ?? $package->base_price ?? 0, 2) }}
                        </span>
                    </div>
                    <h3 class="text-lg font-bold text-white mb-2">{{ $package->name ?? 'Untitled Package' }}</h3>
                    <p class="text-sm text-slate-400 mb-6 line-clamp-3 leading-relaxed">
                        {{ $package->description ?? 'No description provided for this package.' }}
                    </p>
                </div>

                <div class="border-t border-slate-800/80 pt-4 flex items-center justify-between text-xs text-slate-400">
                    <span class="flex items-center gap-1.5">⏱️ {{ $package->duration ?? 'Standard Time' }}</span>
                    <span class="px-2.5 py-1 rounded-full font-medium {{ ($package->status ?? 'Active') == 'Active' ?
                        'bg-emerald-950 text-emerald-400 border border-emerald-800/30' : 'bg-slate-800 text-slate-400' }}">
        ...
        @empty
            <div class="col-span-full text-center py-16 bg-slate-900/40 border border-slate-800/80 rounded-2xl">
                ...
                <p class="text-slate-300 font-medium mb-1">No packages found</p>
                <p class="text-xs text-slate-500">Get started by clicking the "Add New Package" button above.</p>
            </div>
        @endforelse
    </div>
    ...
    <div id="addPackageModal"
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl max-w-lg w-full p-6 shadow-2xl relative">
            ...
            <form action="{{ route('packages.store') }}" method="POST">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-xs font-semibold text-slate-300 mb-1.5 uppercase">Package
                            Name</label>
                        <input type="text" name="name" required
                            ...
                            placeholder="e.g. Gold Detailing Package">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-300 mb-1.5 uppercase">Category</label>
                        <input type="text" name="category" required
                            class="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500"
                            placeholder="e.g. Complete Protection">
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-slate-300 mb-1.5 uppercase">Price
                                ($)</label>
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3"
                            class="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500 resize-none"
                ...
                <div class="flex justify-end gap-3 mt-8 border-t border-slate-800 pt-4">
                    <button type="button" onclick="document.getElementById('addPackageModal').classList.add('hidden')"
                        class="px-5 py-2.5 bg-slate-800 text-slate-300 rounded-xl text-sm font-medium">Cancel</button>
                    <button type="submit"
                        class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-semibold">Save
                        Package</button>
                </div>
            </form>
        </div>
